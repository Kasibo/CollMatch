...
class SENet(nn.Module):
    ...
    def features(self, x):
        x1 = self.layer0(x)  # b 64 80 80
        x2 = self.layer1(x1)  # b 256 80 80
        x3 = self.layer2(x2)  # b 512 40 40
        x4 = self.layer3(x3)  # b 1024 20 20
        x5 = self.layer4(x4)  # b 2048 10 10
    ...
    def decoder_ConvTranspose(self, x1, x2, x3, x4, x5):
        ...
        :rtype: object
        """
        score = self.bn5(self.relu(self.deconv5(x5)))  # b 1024 20 20    32
        score = score + x4
        score = self.bn4(self.relu(self.deconv4(score)))  # b 512 40 40   64
        score = score + x3
        score = self.bn3(self.relu(self.deconv3(score)))  # b 256 80 80   128
        score = score + x2
        score = self.bn2(self.relu(self.deconv2(score)))  # b 128 160 160s    256
        # if refined_features != None:
        #     score = torch.cat((score, F.interpolate(refined_features[-1],score.shape[-2:], mode='bilinear')),dim=1)
        score = self.bn1(self.relu(self.deconv1(score)))  # 8 128 320 320    512
...
#         return x_masked
    ...
    def forward(self, x, need_fp = False, ddp = None):
        """
        :param x2: c = 256
        :param x3: c = 512
        :param x4: c = 1024
        ...
        if need_fp:
            dropout_layers = DynamicDropout(5, ddp['p_st'], ddp['p_ed'], ddp['p_m'], ddp['mode'])
            # for layer in range(1, 6):
            #     print(f"Layer {layer} Dropout Prob: {dropout_layers.get_dropout_prob(layer)}")
            out_fp = self.decoder_ConvTranspose(
                dropout_layers(x1, 1),
                dropout_layers(x2, 2),
                dropout_layers(x3, 3),  
                dropout_layers(x4, 4),  
...
class DynamicDropout(nn.Module):
    def __init__(self, num_layers: int = 5, p_min: float = 0.1, p_max: float = 0.5, alpha: float = 1.0, mode = 'power'): # 'power' or 'cosine' or 'fixed'
        """
        多层动态增强 Dropout
    ...
    def get_dropout_prob(self, layer_idx: int) :
        ...
        if self.mode == 'power':
            ...
            prob = self.p_min + self.p_max * (1 - np.cos(np.pi * (layer_idx - 1)/(self.num_layers - 1))) / 2.0
        else:
            prob = 0.5
...
if __name__ == '__main__':
    ...
    # input.float().cuda()
    outs = net(input, need_fp = True, use_corr = False)
