...
from mmseg.models.utils.masking_transforms import BlockMaskGenerator

class SemiDataset(Dataset):
    def __init__(self, name, root, mode, size=None, id_path=None, nsample=None, bmg = None):
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            file_path = os.path.join(self.root, id.split(' ')[0])
            print(f"Error loading file {file_path}: {e}")
            id = self.ids[random.randint(0, len(self.ids) - 1)]
            img = Image.open(os.path.join(self.root, id.split(' ')[0])).convert('RGB')
        ...
        try:
            ...
            file_path = os.path.join(self.root, random_id.split(' ')[0])
            print(f"Error loading file {file_path}: {e}")
        ...
        mask_generator = BlockMaskGenerator(self.bmg['fst_p'], self.bmg['fst_s'], self.bmg['sec_p'], self.bmg['sec_s'])
        aa = transforms.ToTensor()
        bb = transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        if self.bmg['obj'] == 'strong':
            masked_img = mask_generator.mask_image(aa(img_s2).unsqueeze(0))
        else:
            masked_img = mask_generator.mask_image(aa(img_w).unsqueeze(0))
        masked_img = bb(masked_img[0])

        return normalize(img_w), normalize(img2_w), cutmix_box, normalize(img_s1), normalize(img_s2), \
            normalize(img_s3), normalize(img_s4), normalize(img_s5), masked_img
